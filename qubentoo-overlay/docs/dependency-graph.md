...
│     QubesDB C daemon + libqubesdb (dom0 and domU)                  │
│     Source: QubesOS/qubes-core-qubesdb                             │
│     RDEPEND: app-emulation/xen-tools                               │
│     CONFLICTS: sys-apps/qubes-db (installs same daemon binary)     │
│     OpenRC: qubesdb (sysinit/default, after xenstored)             │
│                                                                    │
├── dev-python/qubesdb  ─────────────────────────────────────────────┤
│     Python bindings for QubesDB                                    │
│     RDEPEND: sys-apps/qubes-core-qubesdb                           │
│                                                                    │
├── sys-apps/qubes-core-vchan-xen ───────────────────────────────────┤
│     vchan transport over Xen shared memory (C library)             │
│     RDEPEND: app-emulation/xen-tools, sys-apps/qubes-core-qubesdb  │
│                                                                    │
...
│              qubes-core-qubesdb, x11-libs/*, dev-python/dbus-python│
│     OpenRC: qubes-gui-daemon (default, after qubesd + DM)          │
│                                                                    │
├── gui-apps/qubes-manager ──────────────────────────────────────────┤
│     PyQt5 dom0 VM management GUI                                   │
│     RDEPEND: sys-apps/qubes-core-admin, dev-python/PyQt5,          │
│              dev-python/qubesadmin, dev-python/qubesdb              │
│     Desktop: /usr/share/applications/qubes-manager.desktop         │
│                                                                    │
└── app-emulation/qubes-vmm-xen-stubdom-linux ───────────────────────┘
      Xen stubdomain kernel + rootfs for PCI passthrough isolation
      RDEPEND: ~app-emulation/xen-${PV}, ~xen-tools-${PV}
      RESTRICT: network-sandbox  OPTIONAL (IOMMU VMs only)
      Installs: /usr/lib/xen/boot/stubdom-linux-{kernel,rootfs}
```
...
│     OpenRC: qubesdb (default, after xenstored)                     │
│                                                                    │
├── dev-python/qubesdb  ─────────────────────────────────────────────┤
...
│     RDEPEND: qubes-core-qubesdb, qubes-libvchan, qubes-rpc-proxy,  │
...
│     nftables-based firewall for NetVMs / AppVMs                    │
│     Source: QubesOS/qubes-core-admin-linux (firewall/ subdir)      │
│     RDEPEND: net-firewall/nftables, sys-apps/qubes-core-agent      │
│     OpenRC: qubes-firewall (default, after qubesdb + qubes-agent)  │
│                                                                    │
└── sys-apps/qubes-input-proxy ──────────────────────────────────────┘
      Input device isolation (sender in dom0/inputVM, receiver in AppVM)
      RDEPEND: sys-apps/qubes-core-agent, dev-libs/libevdev
      udev: 70-qubes-input.rules (/dev/input/* → group qubes)
      OpenRC: qubes-input-proxy (default, after qubesdb)
```
...
default runlevel (inside template/AppVM)
  ...
  ├── qubesdb            (after xenstored)
  │     └── qubes-agent  (after qubesdb + net)
  │           ├── qubes-qrexec-agent  (after qubes-agent)
  │           ├── qubes-firewall      (after qubesdb + qubes-agent; NetVMs/AppVMs)
  │           └── qubes-input-proxy   (after qubesdb; AppVMs with input forwarding)
...
graph TD
    ...
    %% ── QubesDB ──────────────────────────────────────────────────
    qubes_coredb["sys-apps/qubes-core-qubesdb"]
    qubes_coredb --> xen_tools
    py_qubesdb --> qubes_coredb

    %% ── vchan ────────────────────────────────────────────────────
    vchan_xen["sys-apps/qubes-core-vchan-xen"]
    vchan_xen --> xen_tools
    vchan_xen --> qubes_coredb
    ...
    gui_daemon --> libvchan
    gui_daemon --> qubes_coredb

    qubes_manager["gui-apps/qubes-manager"]
    qubes_manager --> core_admin
    qubes_manager --> py_qubesdb

    stubdom["app-emulation/qubes-vmm-xen-stubdom-linux"]
    stubdom --> xen
    stubdom --> xen_tools

    %% ── DOMU / Template ──────────────────────────────────────────
    core_agent["sys-apps/qubes-core-agent"]
    core_agent --> qubes_coredb
    core_agent --> libvchan
    ...
    firewall["net-proxy/qubes-firewall"]
    firewall --> core_agent

    input_proxy["sys-apps/qubes-input-proxy"]
    input_proxy --> core_agent
    ...
    class xen,xen_tools,core_admin,core_linux,rpc_proxy,gui_daemon,qubes_manager,stubdom dom0
    class core_agent,gui_agent,firewall,input_proxy domu
    class qubes_coredb,vchan_xen,libvchan,gui_common shared
...
13. gui-apps/qubes-manager
14. sys-apps/qubes-input-proxy   (USE="sender" in dom0)
15. app-emulation/qubes-vmm-xen-stubdom-linux  (optional, IOMMU/PCI passthrough)

# DOMU / Template (inside chroot)
1.  sys-apps/qubes-core-qubesdb
...
9.  net-proxy/qubes-firewall      (NetVMs and AppVMs)
10. sys-apps/qubes-input-proxy    (USE="receiver" in AppVMs)
```
